...
this_week <- regular_season_schedule %>% 
  dplyr::filter(
    game_start_time > '2019-07-18',
    game_start_time < '2019-07-25'
  )
season_team_feed <- dfstools::bdb_wnba_team(
  "~/Dropbox/19-wnba-team/07-17-2019-wnba-season-team-feed.xlsx"
)
...
season_dfs_feed <- dfstools::bdb_wnba_dfs(
  "~/Dropbox/19-wnba-dfs/07-17-2019-wnba-season-dfs-feed.xlsx"
) %>% dplyr::filter(
...
minutes_cutoff <- quantile(
  draftkings_main_slate$projected_minutes, probs = 0.50, na.rm = TRUE
)
...
draftkings_showdown_slate <- draftkings_projections %>% 
  ...
  dplyr::select(-draftkings_position) 
minutes_cutoff <- quantile(
  draftkings_showdown_slate$projected_minutes, probs = 0.50, na.rm = TRUE
)

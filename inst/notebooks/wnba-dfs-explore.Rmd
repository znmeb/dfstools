...
regular_season_schedule <- regular_season_schedule %>% mutate(
...
season_dfs_feed <- read_excel(
  "~/Dropbox/19-wnba-dfs/06-26-2019-wnba-season-dfs-feed.xlsx",
  skip = 2
)
names(season_dfs_feed) <- .dfs_column_names

player_box_score <- dfstools::player_box_score(
  "~/Dropbox/19-wnba-player/06-26-2019-wnba-season-player-feed.xlsx"
)
...
fanduel_leaderboard <- fanduel_augmented %>% 
  group_by(player) %>% summarize(
    count = n(), 
    p50 = quantile(pct, 0.50, names = FALSE), 
    p75 = quantile(pct, 0.75, names = FALSE),
    p95 = quantile(pct, 0.95, names = FALSE)
  ) %>% 
  arrange(desc(p50))

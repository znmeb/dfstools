...
this_week <- regular_season_schedule %>% 
  dplyr::filter(
    ...
    game_start_time < '2019-07-22'
  )
season_team_feed <- dfstools::bdb_wnba_team(
  "~/Dropbox/19-wnba-team/07-14-2019-wnba-season-team-feed.xlsx"
)
...
season_dfs_feed <- dfstools::bdb_wnba_dfs(
  "~/Dropbox/19-wnba-dfs/07-14-2019-wnba-season-dfs-feed.xlsx"
) %>% dplyr::filter(
...
minutes_cutoff <- quantile(
  fanduel_main_slate$projected_minutes, probs = 0.75, na.rm = TRUE
)
fanduel_main_values <- fanduel_main_rosters %>% 
  ...
  dplyr::filter(projected_minutes > minutes_cutoff)
...
fanduel_single_slate <- fanduel_projections %>% 
  ...
  dplyr::select(-fanduel_position)
minutes_cutoff <- quantile(
  fanduel_single_slate$projected_minutes, probs = 0.75, na.rm = TRUE
)
...
minutes_cutoff <- quantile(
  draftkings_slate$projected_minutes, probs = 0.75, na.rm = TRUE
)

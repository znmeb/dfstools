...
print(value_line <- ggplot(
  data = wnba_season_dfs_feed, 
  aes(fanduel_salary, fanduel_points)) +
    geom_point() + geom_smooth(method = MASS::rlm)
)
...
leaderboard <- wnba_season_dfs_feed %>% 
  ...
  dplyr::summarize(
    count = n(),
    p50 = round(quantile(value, 0.50, names = FALSE), 2),
    p75 = round(quantile(value, 0.75, names = FALSE), 2),
    p95 = round(quantile(value, 0.95, names = FALSE), 2)
  ) %>% 
  dplyr::arrange(desc(p50))

...
player_table <- season_dfs_feed %>% 
  dplyr::group_by(player) %>% 
  dplyr::summarize(
    games = n(),
    minutes = sum(minutes),
    fanduel_points = sum(fanduel_points),
    fanduel_value = sum(fanduel_value),
    draftkings_points = sum(draftkings_points),
    draftkings_value = sum(draftkings_value)
 )
mixture_input <- player_table %>% 
  dplyr::select(
    games,
    minutes,
    fanduel_points,

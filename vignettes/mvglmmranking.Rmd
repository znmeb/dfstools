...
## Downloading the raw data for the current season for a league
1. Choose your league. I'm going to use NBA.
2. Acquire the tibble: `nba_raw <- get_season("nba")`.
...
1. Run the projections: `nba_result <- project_season(nba_raw)`. You'll see the iterations for the `mvglmmRank` model. This will take some time, depending on the number of games in the input dataset.

    The result of `project_season` is a list of three items:
    
    * projections: a tibble containing the projections
    * model: the model `mvglmmRank` derived
...
* home_response_proj: the projected score for the home team
...
This last column needs some explanation. The Shannon entropy is a measure of information - how much we stand to learn about the teams from the outcome of the game. It's an indicator of the projected closeness of the game. The maximum value, 1.0, indicates that the game is projected to be a tie and we'll learn a lot from the outcome.
